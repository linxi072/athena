...
public class ImgDdUploadUtils {
    ...
    public static Long appendUpload2Server(HttpServletRequest request) {
        try {
            ...
            String lastModifyTime = request.getParameter("lastModifyTime");
            //文件后缀
            String filela = fileName.substring(fileName.lastIndexOf("."));
            //文件前缀
            String filef = fileName.substring(0, fileName.lastIndexOf("."));
...
//            File file = new File(currentFilePath+fileName+"."+lastModifyTime);
            ...
            if (currentFileLength == totalSize) {
                ...
                // 如果存在形如test.txt的文件，则新的文件存储为test+当前时间戳.txt, 没处理不带扩展名的文件
                if (newFile.exists()) {
